...
void addAppointment(int patientID) {
    struct Patient* pat = retrieve_patient(patientID);
    if (pat == NULL) {
...
void displayAllAppointments() {
    ...
    while (current != NULL) {
        printf("%-8d %-15s %-15s %-12s %-10s %-10s\n",
               ...
               current->patientPtr->name,
               current->doctorPtr->name,
               current->date,
               current->time,
               current->status);
        current = current->next;
...
void saveAppointments() {
    ...
    while (curr) {
        ...
        fprintf(f, "%d|%d|%d|%s|%s|%s\n",
                curr->appointmentID, pID, dID, curr->date, curr->time, curr->status);
        curr = curr->next;
    }
    fclose(f);
}
...
void loadAppointments() {
    ...
    while (!feof(f)) {
        ...
        int pID, dID;
        if (fscanf(f, "%d|%d|%d|%[^|]|%[^|]|%[^\n]\n",
            &a->appointmentID, &pID, &dID, a->date, a->time, a->status) == 6) {

            a->patientPtr = retrieve_patient(pID); // Re-linking
            a->doctorPtr = searchDoctorByID(dID);   // Re-linking
            a->next = NULL;
